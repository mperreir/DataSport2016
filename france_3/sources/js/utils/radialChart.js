...
function RadialProgressChart(query, options) {
  ...
  var series = self.options.series
    , width = 40 + ((self.options.diameter / 2) + (self.options.stroke.width * self.options.series.length) + (self.options.stroke.gap * self.options.series.length - 1)) * 2
    , height = width
    , τ = 2 * Math.PI
    , inner = []
    , outer = [];
  ...
  self.field = self.svg.selectAll("g")
    ...
    self.field.on("mouseover", function(item) {
        self.options.hover(item);
    });
    self.field.on("mouseleave", function(item) {
        self.options.leave(item);
    });

  self.field.append("path").attr("class", "progress").attr("filter", "url(#" + dropshadowId +")").attr("data-value",
  function(item) {
      return item.value;
  });
