...
module.exports = exports = function(next) {
    ...
    var materialOcean = new THREE.MeshLambertMaterial({ color: new THREE.Color("#2b4c7e") });
    var ocean = new THREE.Mesh(new THREE.SphereGeometry(49.25 / 50 * global.RADIUS, global.RADIUS, global.RADIUS), materialOcean);
    this.earth.add(ocean);
    
    var materialContinent = new THREE.MeshLambertMaterial({ color: new THREE.Color("#3d8c6a") });
    var continent = this.assets.globe;
    continent.scale.set(global.RADIUS, global.RADIUS, global.RADIUS);
    continent.position.z = 0;
    continent.traverse(function (child) {
    	if (child instanceof THREE.Mesh) {
    		child.material = materialContinent;
    	}
	});
